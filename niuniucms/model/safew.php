...
function safew_table_ready()
{
    ...
    global $db;
    $pre = $db->tablepre;
    $like = addslashes($pre . 'safew_chat');
    $row = db_sql_find_one("SHOW TABLES LIKE '{$like}'", $db);
    if (empty($row)) {
        $ok = false;
        return false;
    }
    $like2 = addslashes($pre . 'safew_push_log');
    $row2 = db_sql_find_one("SHOW TABLES LIKE '{$like2}'", $db);
    $ok = !empty($row2);
    if ($ok) {
        safew_ensure_bot_columns();
    }
    return $ok;
}

function safew_norm_bot_id($bot_id = '')
{
...
function safew_ensure_bot_columns()
{
    global $db;
    $pre = $db->tablepre;
    $chat = "`{$pre}safew_chat`";
    $log = "`{$pre}safew_push_log`";
...
function safew_install_tables()
{
    global $db;
    $pre = $db->tablepre;
    $sql1 = "CREATE TABLE IF NOT EXISTS `{$pre}safew_chat` ("
    ...
    $sql2 = "CREATE TABLE IF NOT EXISTS `{$pre}safew_push_log` ("
    ...
    db_exec($sql2);
    safew_ensure_bot_columns();
    return safew_table_ready();
}

function safew_chat_upsert($chat, $now = 0, $bot_id = 'default')
{
    global $time;
    if (!is_array($chat)) {
        return false;
    ...
    $bot_id = safew_norm_bot_id($bot_id);
    ...
    $old = db_sql_find_one("SELECT id FROM `{$GLOBALS['db']->tablepre}safew_chat` WHERE bot_id='{$safe_bot_id}' AND chat_id='{$safe_chat_id}' LIMIT 1");
    $data = array(
        'chat_type' => $chat_type,
        'title' => $title,
        'username' => $username,
    ...
    if ($old && !empty($old['id'])) {
        return db_update('safew_chat', array('id' => intval($old['id'])), $data);
    }
    $data['bot_id'] = $bot_id;
    $data['chat_id'] = $chat_id;
    $data['create_date'] = $now;
    return db_insert('safew_chat', $data);
}

function safew_chat_list_enabled($bot_id = 'default')
{
    global $db;
    $pre = $db->tablepre;
    $bot_id = addslashes(safew_norm_bot_id($bot_id));
    $sql = "SELECT * FROM `{$pre}safew_chat` WHERE bot_id='{$bot_id}' AND enabled=1 ORDER BY id ASC";
    $rows = db_sql_find($sql);
    return is_array($rows) ? $rows : array();
}

function safew_api_call($token, $method, $payload = array(), &$raw = '')
{
    ...
    $url = "https://api.safew.org/bot{$token}/{$method}";
...
function safew_upload_photo_file($token, $chat_id, $caption, $photo_url, &$resp = array(), $reply_markup = '', $parse_mode = '')
{
    $token = trim((string) $token);
    ...
    $tmp = tempnam(sys_get_temp_dir(), 'swp_');
    if ($tmp === false) {
    ...
    $ch = curl_init();
    $url = "https://api.safew.org/bot{$token}/sendPhoto";
    $post = array(
        'chat_id' => $chat_id,
...
function safew_send_photo_best_effort($token, $chat_id, $photo, $caption, &$resp = array(), $reply_markup = '', $parse_mode = '')
{
    $payload = array(
    ...
    $resp = safew_api_call($token, 'sendPhoto', $payload, $raw);
    if (array_value($resp, 'ok', false)) {
        return true;
    }
    if (!safew_is_absolute_url($photo)) {
        return false;
    }
    ...
    $upload_resp = array();
    $ok = safew_upload_photo_file($token, $chat_id, $caption, $photo, $upload_resp, $reply_markup, $parse_mode);
    if ($ok) {
        $resp = $upload_resp;
        return true;
    }
...
function safew_sync_updates($token, $limit = 100, $bot_id = 'default')
{
    global $time;
    if (!safew_table_ready() && !safew_install_tables()) {
        return array('ok' => false, 'message' => 'table not ready');
    }
    $bot_id = safew_norm_bot_id($bot_id);
    $limit = max(1, min(100, intval($limit)));
    $offset = intval(kv_get('safew_last_update_id_' . $bot_id));
    $payload = array(
        'offset' => $offset > 0 ? $offset + 1 : 0,
        'limit' => $limit,
        'timeout' => 0,
    );
    $resp = safew_api_call($token, 'getUpdates', $payload, $raw);
    if (!array_value($resp, 'ok', false)) {
    ...
    foreach ((array) $rows as $u) {
        ...
        if (!empty($chat) && safew_chat_upsert($chat, (int) $time, $bot_id) !== false) {
            $saved++;
        }
    }
    if ($max_update_id > $offset) {
        kv_set('safew_last_update_id_' . $bot_id, strval($max_update_id));
    }
    return array(
        'ok' => true,
...
function safew_is_absolute_url($url)
{
    $url = trim((string) $url);
    return $url !== '' && preg_match('#^https?://#i', $url);
}

function safew_build_abs_url($base_url, $path_or_url)
{
    $u = trim((string) $path_or_url);
    if ($u === '') {
        return '';
    }
    if (safew_is_absolute_url($u)) {
        return $u;
    }
...
function safew_build_vod_link($base_url, $vid, $conf, $agent_code = '')
{
    $base = rtrim(trim((string) $base_url), '/');
    $vid = intval($vid);
    if ($base === '' || $vid <= 0) {
    ...
    $route_vod = trim((string) array_value(array_value($conf, 'route', array()), 'vod', 'vod'));
    if ($route_vod === '') {
        $route_vod = 'vod';
    }
    $agent_code = strtoupper(trim((string) $agent_code));
    $agent_qs = '';
    if ($agent_code !== '' && preg_match('/^[A-Z0-9_\\-]{4,32}$/', $agent_code)) {
        $agent_qs = 'agent=' . urlencode($agent_code);
    }
    $rewrite = intval(array_value($conf, 'url_rewrite_on', 0));
    if ($rewrite > 1) {
        $url = $base . '/' . $route_vod . '/' . $vid;
        if ($agent_qs !== '') {
            $url .= (strpos($url, '?') === false ? '?' : '&') . $agent_qs;
        }
        return $url;
    }
    $url = $base . '/?0=' . urlencode($route_vod) . '&1=' . $vid;
    if ($agent_qs !== '') {
        $url .= '&' . $agent_qs;
    }
    return $url;
...
function safew_brief_text($text, $max_len = 80)
{
    ...
    $max_len = max(20, min(200, intval($max_len)));
...
function safew_parse_tag_pool($raw, $limit = 12)
{
    $raw = trim((string) $raw);
    if ($raw === '') {
        return array();
...
function safew_build_tag_line($raw_pool, $limit = 12)
{
    $tags = safew_parse_tag_pool($raw_pool, $limit);
    if (empty($tags)) {
        return '';
    }
    return implode(' ', $tags);
...
function safew_build_tag_line_spoiler_html($raw_pool, $limit = 12)
{
    $tags = safew_parse_tag_pool($raw_pool, $limit);
    if (empty($tags)) {
        return '';
    ...
    $ret = array();
    foreach ($tags as $t) {
        $ret[] = '<tg-spoiler>' . htmlspecialchars((string) $t, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '</tg-spoiler>';
    }
    return implode(' ', $ret);
}

function safew_parse_menu_buttons($raw, $max = 8)
{
    $ret = array();
    $arr = $raw;
    if (!is_array($arr)) {
...
function safew_build_reply_markup($raw_buttons, $max = 8)
{
    $buttons = safew_parse_menu_buttons($raw_buttons, $max);
    if (empty($buttons)) {
        return '';
    }
    $rows = array();
    foreach ($buttons as $btn) {
        $rows[] = array(array('text' => $btn['text'], 'url' => $btn['url']));
    }
    $markup = array('inline_keyboard' => $rows);
    return json_encode($markup, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
}

function safew_push_log_exists($chat_id, $msg_key, $bot_id = 'default')
{
    global $db;
    $pre = $db->tablepre;
    $bot_id = addslashes(safew_norm_bot_id($bot_id));
    ...
    $row = db_sql_find_one("SELECT id FROM `{$pre}safew_push_log` WHERE bot_id='{$bot_id}' AND chat_id='{$cid}' AND msg_key='{$mk}' AND status=1 LIMIT 1");
    return !empty($row);
}

function safew_push_log_add($chat_id, $vid, $msg_key, $status, $error = '', $bot_id = 'default')
{
    global $time;
    $bot_id = safew_norm_bot_id($bot_id);
    $row = array(
    ...
    $cid = addslashes((string) $chat_id);
    $mk = addslashes((string) $msg_key);
    $old = db_sql_find_one("SELECT id FROM `{$pre}safew_push_log` WHERE bot_id='{$safe_bot_id}' AND chat_id='{$cid}' AND msg_key='{$mk}' LIMIT 1");
    if ($old && !empty($old['id'])) {
        return db_update('safew_push_log', array('id' => intval($old['id'])), array(
            'vid' => intval($vid),
            'status' => intval($status),
            'error_msg' => substr((string) $error, 0, 500),
            'create_date' => (int) $time,
    ...
    return db_insert('safew_push_log', $row);
}

function safew_push_new_vod($token, $base_url, $limit = 10, $bot_id = 'default')
{
    global $db, $time, $conf;
    if (!safew_table_ready() && !safew_install_tables()) {
        return array('ok' => false, 'message' => 'table not ready');
    ...
    $chat_list = safew_chat_list_enabled($bot_id);
    if (empty($chat_list)) {
        return array('ok' => true, 'message' => 'no enabled chat', 'sent' => 0);
    }
    $last_ts = intval(kv_get('safew_last_push_ts_' . $bot_id));
    if ($last_ts <= 0) {
    ...
    $sent = 0;
    $failed = 0;
    $push_agent_code = strtoupper(trim((string) array_value($conf, 'safew_push_agent_code', '')));
    $tag_spoiler_on = intval(array_value($conf, 'safew_tag_spoiler_on', 0)) ? 1 : 0;
    $tag_pool_raw = (string) array_value($conf, 'safew_tag_pool', '');
    $reply_markup = safew_build_reply_markup((string) array_value($conf, 'safew_menu_buttons', ''), 8);
    foreach ($rows as $v) {
        ...
        $link = safew_build_vod_link($base_url, $vid, $conf, $push_agent_code);
        $poster = safew_build_abs_url($base_url, array_value($v, 'pic', ''));
        ...
        $tag_line = safew_build_tag_line($tag_pool_raw, 12);
        $tag_line_spoiler = $tag_spoiler_on ? safew_build_tag_line_spoiler_html($tag_pool_raw, 12) : '';
        $parse_mode = '';
        ...
        foreach ($chat_list as $chat) {
            ...
            if (safew_push_log_exists($chat_id, $msg_key, $bot_id)) {
                continue;
            }
            if ($poster !== '') {
                $resp = array();
                $ok_photo = safew_send_photo_best_effort($token, $chat_id, $poster, $caption, $resp, $reply_markup, $parse_mode);
                if (!$ok_photo) {
                    $payload2 = array(
                        'chat_id' => $chat_id,
                        'text' => $caption,
                    ...
                    $resp2 = safew_api_call($token, 'sendMessage', $payload2, $raw2);
                ...
                $resp = safew_api_call($token, 'sendMessage', $payload, $raw);
            }
            if (array_value($resp, 'ok', false)) {
                safew_push_log_add($chat_id, $vid, $msg_key, 1, '', $bot_id);
                $sent++;
            } else {
                safew_push_log_add($chat_id, $vid, $msg_key, 0, array_value($resp, 'description', 'send failed'), $bot_id);
                $failed++;
            }
        }
    }
    if ($max_ts > $last_ts) {
        kv_set('safew_last_push_ts_' . $bot_id, strval($max_ts));
    }
    return array(
        'ok' => true,
        'sent' => $sent,
