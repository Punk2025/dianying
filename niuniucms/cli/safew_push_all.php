<?php
/**
 * SafeW 多机器人推送 worker（读取后台启用机器人池）
 *
 * 用法：
 * php /www/wwwroot/sesewu3.eu.cc/niuniucms/cli/safew_push_all.php [limit] [--force]
 */
if (PHP_SAPI !== 'cli') {
    exit("CLI only\n");
}
...
if (!function_exists('safew_push_new_vod')) {
...
$force = false;
$limit = intval(array_value($conf, 'safew_push_limit', 10));
...
$interval_min = max(1, min(1440, intval(array_value($conf, 'safew_push_interval_min', 5))));
if (!$force) {
    $now = time();
    $last = intval(kv_get('job_last_safew_push_all_ts'));
...
if ($base_url === '') {
    echo json_encode(array('ok' => false, 'message' => 'safew_base_url empty'), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) . PHP_EOL;
    exit(1);
}
...
if ($summary['ok']) {
    kv_set('job_last_safew_push_all_ts', strval(time()));
}
exit($summary['ok'] ? 0 : 1);
