<?php
/**
 * Telegram 多机器人同步 worker（读取后台启用机器人池）
 *
 * 用法：
 * php /www/wwwroot/sesewu3.eu.cc/niuniucms/cli/tg_sync_all.php [limit] [--force]
 */
if (PHP_SAPI !== 'cli') {
    exit("CLI only\n");
}
...
$force = false;
$limit = intval(array_value($conf, 'tg_sync_limit', 100));
...
$manual_force = intval(kv_get('job_manual_tg_sync_force'));
$manual_limit = intval(kv_get('job_manual_tg_sync_limit'));
if ($manual_force === 1) {
    $force = true;
    if ($manual_limit > 0) {
        $limit = max(1, min(100, $manual_limit));
    }
    kv_set('job_manual_tg_sync_force', '0');
    kv_set('job_manual_tg_sync_limit', '0');
    kv_set('job_manual_tg_sync_enqueue_ts', '0');
}
$interval_min = max(1, min(1440, intval(array_value($conf, 'tg_sync_interval_min', 1))));
if (!$force) {
    $now = time();
    $last = intval(kv_get('job_last_tg_sync_all_ts'));
...
if ($summary['ok']) {
    kv_set('job_last_tg_sync_all_ts', strval(time()));
}
exit($summary['ok'] ? 0 : 1);
